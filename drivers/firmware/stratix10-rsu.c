...
/**
 * rsu_async_status_callback() - Status callback from Intel Service Layer
			      ...
			      rsu_async_callback callback)
{
	...
	if (ret > 0)
		...
		dev_warn(dev, "Timeout occurred. Trying to poll the response\n");
			    ...
			    const char *buf, size_t count)
{
	...
	if (priv->async) {
		...
		ret = rsu_send_msg(priv, COMMAND_RSU_STATUS, 0,
				   rsu_status_callback);
		/**In asynchronous operation COMMAND_RSU_RETRY is integrated to
		 * be part of COMMAND_RSU_STATUS.so we need to invoke it only
		 * in synchronous mode.
		 */
		if (!ret)
...
static int stratix10_rsu_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		if (priv->async) {
			stratix10_svc_remove_async_client(priv->chan);
		} else if (priv->get_spt_response_buf) {
