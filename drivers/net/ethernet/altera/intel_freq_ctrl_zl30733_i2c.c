...
static int zl30733_dpll_wait_for_df_ctrl_readsem(struct i2c_client * i2c_cli)
{
	...
	do {
		...
		ret = i2c_zl30733_read_byte_data(i2c_cli, ZL30733_PAGE5_REG_DPLL_DF_READ_0, &ctrl_val, 1);
		if (ret) {
		...
		if (ZL30733_DPLL_DF_CTRL_SEM_GET(ctrl_val) == 0) {
			dev_dbg(&i2c_cli->dev,"%s: pll: ZL30733_PAGE5_REG_DPLL_DF_READ_0[0x%x] ""read : 0x%x\n", __func__, ZL30733_PAGE5_REG_DPLL_DF_READ_0,ctrl_val);
			
			ret = FREQ_CTRL_ERROR_SUCCESS;
			goto zl_dpll_readsem_err;
		}

		dev_info(&i2c_cli->dev,"%s: pll: ZL30733_PAGE5_REG_DPLL_DF_READ_0[0x%x] "
		 "read : 0x%x attempt %d\n", __func__,
		 ZL30733_PAGE5_REG_DPLL_DF_READ_0, ctrl_val, attempts);
...
void intel_freq_control_zl30733(struct work_struct *work)
{
	...
	/* dpll_df_offset_2 contains a 2's complement binary value of delta frequency offset.
	   ...
	   Note 4: The read value of this register during NCO mode is the value latched just prior to entering NCO mode.
	*/
...
static void zl30733_check_dpll(struct i2c_client *i2c_cli)
{
	...
	for (loop = 0; loop < 10000; ++loop) {
		if (i2c_zl30733_read_byte_data(i2c_cli, ZL30733_REG_DPLL_MON_STATUS(i), &buf, 1) == 0) {
			if (buf & 1) {
				pr_info("%s: ZL30733_REG_DPLL_MON_STATUS(%u) : 0x%02x - OK, lock\n",__func__, i, buf);
				return;
			}
		}
		mdelay(10);
