...
{
	const u8 blk_align = (rvld_msb & ETH_PHY_PTP_MSB_BLK_ALIGN_OCC_MASK) >> ETH_PHY_PTP_MSB_BLK_ALIGN_OCC_SHIFT;
...
static int eth_ftile_tx_rx_user_flow(intel_fpga_xtile_eth_private *priv)
{
	...
	regval = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_tx_lane_calc_data_constdelay));
	tx_const_delay      = regval & 0x7FFFFFFF;
	tx_const_delay_sign = regval >> 31;
	n = eth_soft_csroffs(ptp_tx_lane1_calc_data_offset) - eth_soft_csroffs(ptp_tx_lane0_calc_data_offset);
	for (pl = 0; pl < num_pl; pl++) {
		regval = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_tx_lane0_calc_data_offset) + n * pl);
		tx_apulse_offset[pl]      = regval & 0x7FFFFFFF;
		tx_apulse_offset_sign[pl] = regval >> 31;
		tx_apulse_wdelay[pl]      = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_tx_lane0_calc_data_wiredelay) + n * pl) & 0xFFFFF;
		tx_apulse_time[pl]        = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_tx_lane0_calc_data_time) + n * pl) & 0x0FFFFFFF;
	...
	if (num_pl > 1) {
		...
		for (pl = 0; pl < num_pl; pl++) {
			if (tx_apulse_time_max - tx_apulse_time[pl] > 0x01F40000) { // 500 ns
				// if diff is above 500 ns, we assume rollover (normally diff is below 10 ns)
				if ((tx_apulse_time_max >> 24) == 0xF)
					// natural rollover bit 27 to 28, so add bit 28
					tx_apulse_time[pl] += 0x10000000;
				else
					// TOD 1 billion rollover 48'3B9A_CA00_0000, so add 0xA000000
					tx_apulse_time[pl] += 0x0A000000;
			}
		}
		/* Step 3b Calculate actual time of TX Alignment Marker at TX PMA parallel data interface */
		for (pl = 0; pl < num_pl; pl++) {
			...
			/* TX reference lane is the TX physical lane where tx_am_actual_time is the largest for all physical lanes. */
	...
	// TX extra latency is a positive adjustment, set most-significant bit of the register to 0 to indicate positive.
	...
	if (speed > 25) {
		...
		*/
		...
		if (!strcasecmp(priv->fec_type, "kp-fec") || !strcasecmp(priv->fec_type, "ll-fec")) {
			// KP-FEC/LL-FEC variants:
			for (vl = 0; vl < num_vl; vl++)
				tx_vl_offset[vl] = ((u64)(vl - (vl % num_pl)) / num_pl * 68 * ui_value) >> 12;
		} else if (!strcasecmp(priv->fec_type, "kr-fec")) {
			// KR-FEC variants:
			for (vl = 0; vl < num_vl; vl++)
				tx_vl_offset[vl] = ((u64)(vl - (vl % num_pl)) / num_pl * 66 * ui_value) >> 12;
		} else if (!strcasecmp(priv->fec_type, "no-fec")) {
			// No FEC variants:
			for (vl = 0; vl < num_vl; vl++)
				tx_vl_offset[vl] = ((u64)(vl - (vl % num_pl)) / num_pl * 1 * ui_value) >> 12;
		}
	}
	...
	// User should perform TX UI adjustment of hard PTP IP from time to time to prevent time counter drift from golden time-of-day in the system.
	...
	regval = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_rx_lane_calc_data_constdelay));
	rx_const_delay      = regval & 0x7FFFFFFF;
	rx_const_delay_sign = regval >> 31;
	n = eth_soft_csroffs(ptp_rx_lane1_calc_data_offset) - eth_soft_csroffs(ptp_rx_lane0_calc_data_offset);
	for (pl = 0; pl < num_pl; pl++) {
		regval = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_rx_lane0_calc_data_offset) + n * pl);
		rx_apulse_offset[pl]      = regval & 0x7FFFFFFF;
		rx_apulse_offset_sign[pl] = regval >> 31;
		rx_apulse_wdelay[pl]      = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_rx_lane0_calc_data_wiredelay) + n * pl) & 0xFFFFF;
		rx_apulse_time[pl]        = hssi_csrrd32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(ptp_rx_lane0_calc_data_time) + n * pl) & 0x0FFFFFFF;
	...
	*/
	if (strcasecmp(priv->fec_type, "no-fec") != 0) {
		// FEC variants:
		for (fl = 0; fl < num_fl; fl++) {
			if ((rx_xcvr_if_pulse_adj[fl] + (rx_xcvr_if_pulse_adj[fl - (fl % pl_fl_map)] & 0x1f)) > rx_xcvr_if_pulse_adj[fl - (fl % pl_fl_map)]) {
				rx_spulse_offset[fl] = ((u64)(rx_xcvr_if_pulse_adj[fl] - rx_xcvr_if_pulse_adj[fl - (fl % pl_fl_map)] +
							      (rx_xcvr_if_pulse_adj[fl - (fl % pl_fl_map)] & 0x1f)) * ui_value * pl_fl_map) >> 12;
				rx_spulse_offset_sign[fl] = 0;
			} else {
				rx_spulse_offset[fl] = ((u64)(rx_xcvr_if_pulse_adj[fl - (fl % pl_fl_map)] - rx_xcvr_if_pulse_adj[fl] -
							      (rx_xcvr_if_pulse_adj[fl - (fl % pl_fl_map)] & 0x1f)) * ui_value * pl_fl_map) >> 12;
		...
		for (vl = 0; vl < num_vl; vl++) {
			...
			if (speed == 50) {
				final_offs = (get_gb_33_66_occupancy(speed, rvld_lsb[vl], rvld_msb[vl]) +
					      ...
					      2 * get_am_detect_occupancy(speed, rvld_lsb[vl], rvld_msb[vl]) +
					      2 * 66 * (((rvld_lsb[vl] & ETH_PHY_PTP_LSB_AM_COUNT_MASK) >> ETH_PHY_PTP_LSB_AM_COUNT_SHIFT) |
							(((rvld_lsb[vl] & ETH_PHY_PTP_LSB_SPARE_MASK) >> ETH_PHY_PTP_LSB_SPARE_SHIFT) << 2)));
				if (local_vl == 1 || local_vl == 3)
					final_offs -= 1;
			} else { // speed == 100
				final_offs = (get_gb_33_66_occupancy(speed, rvld_lsb[vl], rvld_msb[vl]) +
					      get_gb_66_110_occupancy(speed, rvld_lsb[vl], rvld_msb[vl]) +
					      5 * get_blk_align_occupancy(speed, rvld_lsb[vl], rvld_msb[vl]) +
					      5 * get_am_detect_occupancy(speed, rvld_lsb[vl], rvld_msb[vl]) +
					      5 * 66 * (((rvld_lsb[vl] & ETH_PHY_PTP_LSB_AM_COUNT_MASK) >> ETH_PHY_PTP_LSB_AM_COUNT_SHIFT) |
							(((rvld_lsb[vl] & ETH_PHY_PTP_LSB_SPARE_MASK) >> ETH_PHY_PTP_LSB_SPARE_SHIFT) << 2)));
				if (local_vl == 1 || local_vl == 6 || local_vl == 11 || local_vl == 16)
					final_offs -= 1;
				else if (local_vl == 2 || local_vl == 7 || local_vl == 12 || local_vl == 17)
					final_offs -= 2;
				else if (local_vl == 3 || local_vl == 8 || local_vl == 13 || local_vl == 18)
					final_offs -= 3;
				else if (local_vl == 4 || local_vl == 9 || local_vl == 14 || local_vl == 19)
					final_offs -= 4;
	...
	if (speed == 10 || speed == 25) {
		...
		// rx_apulse_time[pl] represents async pulse time of each physical lane in 28 bit format,
		// where bit [27:16] is ns while bit [15:0] is fns. There are 2 types of rollover possibly happening:
		// 1. Natural rollover from bit 27 to bit 28 when the value is reaching 28'hFFF_FFFF. Bit[27:24] before rollover are all 1s'
		// 2. Billion rollover when TOD is reaching 1 billion ns or 48'3B9A_CA00_0000. Bit[27:24] before rollover is 4'h9.
		...
		/* Step 5c Calculate actual time of RX Alignment Marker at RX PMA parallel data interface */
		...
		if (strcasecmp(priv->fec_type, "no-fec") != 0) {
			...
			for (fl = 0; fl < num_fl; fl++) {
				...
				rx_am_actual_time[fl] = ((rx_apulse_time[pl])
							 + (rx_apulse_offset_sign[pl] ? -rx_apulse_offset[pl] : rx_apulse_offset[pl])
							 - (rx_apulse_wdelay[pl])
			...
			for (vl = 0; vl < num_vl; vl++) {
				regval = rvld_msb[vl];
				pl = (regval & ETH_PHY_PTP_MSB_LOCAL_PL_MASK) >> ETH_PHY_PTP_MSB_LOCAL_PL_SHIFT;
		...
		/* RX reference lane, rx_ref_pl is the RX physical lane associated to RX FEC/virtual lane
		   which its rx_am_actual_time is the largest among all FEC/virtual lanes. */
		if (strcasecmp(priv->fec_type, "no-fec") != 0) {
			// FEC variants:
			// rx_am_actual_time_max is maximum value, found for rx_am_actual_time[rx_ref_fl];
			rx_ref_pl = (rx_ref_fl - (rx_ref_fl % pl_fl_map)) / pl_fl_map;
		} else {
			// No FEC variants:
			// rx_am_actual_time_max is maximum value, found for rx_am_actual_time[rx_ref_vl]
			regval = rvld_msb[rx_ref_vl];
			rx_ref_pl = (regval & ETH_PHY_PTP_MSB_LOCAL_PL_MASK) >> ETH_PHY_PTP_MSB_LOCAL_PL_SHIFT;
		}
	}
	...
	*/
	...
	// RX extra latency is a negative adjustment, set most-significant bit of the register to 1 to indicate negative.
	...
	if (speed > 25) {
		...
		if (!strcasecmp(priv->fec_type, "kp-fec") || !strcasecmp(priv->fec_type, "ll-fec")) {
			// KP-FEC/LL-FEC variants:
			for (vl = 0; vl < num_vl; vl++)
				rx_vl_offset[vl] = ((u64)(vl - (vl % num_pl)) / num_pl * 68 * ui_value) >> 12;
		} else if (!strcasecmp(priv->fec_type, "kr-fec")) {
			// KR-FEC variants:
			for (vl = 0; vl < num_vl; vl++)
	...
	// User should perform RX UI adjustment of hard PTP IP from time to time to prevent time counter drift from golden time-of-day in the system.
	// Done via timer_setup at end of this function
...
int ftile_init_mac(intel_fpga_xtile_eth_private *priv)
{
	 ...
	 */
	hssi_csrwr32_ba( pdev, HSSI_ETH_RECONFIG, chan,eth_soft_csroffs(eth_reset),ETH_SOFT_TX_RST | ETH_SOFT_RX_RST | ETH_EIO_SYS_RST);

	/* Step 2 - De-assert TX digital reset
	 *	EHIP CSR Write, Offset = 0x310, value = 0x4
	 */
	hssi_csrwr32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(eth_reset),ETH_SOFT_RX_RST);

	/* Step 3 - De-assert RX digital reset
	 *	EHIP CSR Write, Offset = 0x310, value = 0x0
	 */
	hssi_csrwr32_ba(pdev, HSSI_ETH_RECONFIG, chan, eth_soft_csroffs(eth_reset),0x0);

	/* Step 4 - Verify RX PCS Status
