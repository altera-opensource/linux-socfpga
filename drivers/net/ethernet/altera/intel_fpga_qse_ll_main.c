...
static int alloc_init_skbufs(struct intel_fpga_qse_private *priv)
{
 ...
 */
static int qse_tx_complete(struct intel_fpga_qse_private *priv)
{
 ...
 */
static int qse_poll(struct napi_struct *napi, int budget)
{
 ...
 */
static irqreturn_t intel_fpga_isr(int irq, void *dev_id)
{
 ...
 */
static int qse_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct intel_fpga_qse_private *priv = netdev_priv(dev);
	unsigned int txsize = priv->dma_priv.tx_ring_size;
	unsigned int entry;
...
static void qse_update_mac_addr(struct intel_fpga_qse_private *priv, const unsigned char *addr)
{
	u32 msb;
	u32 lsb;
 ...
 */
static void intel_fpga_qse_ll_remove(struct platform_device *pdev)
{
