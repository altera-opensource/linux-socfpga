 ...
 */
static int etile_tx_complete(struct intel_fpga_etile_eth_private *priv)
{
 ...
 */
static int etile_poll(struct napi_struct *napi, int budget)
{
 ...
 */
static irqreturn_t intel_fpga_etile_isr(int irq, void *dev_id)
{
 ...
 */
static netdev_tx_t etile_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct intel_fpga_etile_eth_private *priv = netdev_priv(dev);
	unsigned int txsize = priv->dma_priv.tx_ring_size;
	unsigned int entry;
 ...
 */
static void intel_fpga_etile_remove(struct platform_device *pdev)
{
