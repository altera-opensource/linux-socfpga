...
static int altera_quadspi_wait_till_ready(struct altera_quadspi *q)
{
	unsigned long finish;
	u8 status;
	int ret = 0;

	finish = jiffies + QUADSPI_MAX_TIME_OUT;
	do {
		ret = altera_quadspi_read_reg(q, NOR_OP_RDID, &status, 1);
		if (ret)
			return ret;
	...
	dev_err(q->dev, "quadspi controller is busy, timeout\n");
	return -EBUSY;
}
...
static int altera_quadspi_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (len == mtd->size) {
		...
		ret = altera_quadspi_wait_till_ready(q);
		if (ret)
			goto erase_err;
...
static int altera_quadspi_lock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
	struct altera_quadspi_flash *flash = get_flash_data(mtd);
	struct altera_quadspi *q = mtd->priv;
	uint32_t offset = ofs;
...
static int altera_quadspi_unlock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
	struct altera_quadspi_flash *flash = get_flash_data(mtd);
	struct altera_quadspi *q = mtd->priv;
	u32 mem_op;
...
static int altera_quadspi_scan(struct altera_quadspi *q, const char *name)
{
	...
	ret = altera_quadspi_read_reg(q, NOR_OP_RDID, &id, 1);
	if (ret)
		return -EINVAL;
